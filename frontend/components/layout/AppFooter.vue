    ...
    <div class="container mx-auto px-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-12 mb-10">
        ...
        <div class="md:col-span-12 lg:col-span-5 flex flex-col gap-5">
          <div class="space-y-3">
            ...
            <p :class="isHomePage ? 'text-slate-400' : 'text-slate-500'" class="text-[13px] leading-relaxed max-w-sm">
          ...
          <div class="flex flex-col gap-1">
            ...
            <a :href="`tel:${settings?.hotline || ''}`"
              :class="isHomePage ? 'text-white hover:text-brand-400' : 'text-slate-800 hover:text-brand-600'" class="text-lg font-black tracking-tight transition-colors">
              {{ settings?.hotline || '' }}
            </a>
          </div>
        </div>
            ...
            <ul :class="isHomePage ? 'text-slate-400' : 'text-slate-500'" class="space-y-2.5 text-[13px] font-medium">
              <li v-for="item in group.items" :key="item.id">
                <NuxtLink :to="menuLink(item)"
                  :class="isHomePage ? 'hover:text-brand-400' : 'hover:text-brand-600'" class="transition-colors duration-200 block">
      ...
      <div :class="isHomePage ? 'border-slate-800' : 'border-slate-200/70'" class="pt-6 border-t flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-xs" :class="isHomePage ? 'text-slate-500' : 'text-slate-400'">
        ...
        <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
          <NuxtLink :to="localePath('/privacy')" :class="isHomePage ? 'hover:text-brand-400' : 'hover:text-brand-600'" class="hover:underline transition-colors">隐私政策</NuxtLink>
          <span :class="isHomePage ? 'bg-slate-700' : 'bg-slate-300'" class="inline-block w-1 h-1 rounded-full"></span>
          <NuxtLink :to="localePath('/terms')" :class="isHomePage ? 'hover:text-brand-400' : 'hover:text-brand-600'" class="hover:underline transition-colors">法律声明</NuxtLink>
